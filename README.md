...
`git status` выводит информацию о статусах файлов в репозитории, если изменений не было после последнего коммита, то файлы не будут указаны в сообщении, их статус `tracked`

*`untracked` неотслеживаемый файл. Файл только появился в репозитории и еще не указывался в команде `git add`

*`staged` - подготовленный. После выполнения команды `git add` файл попадает в staging area (от англ. stage — «сцена», «этап [процесса]» и area — «область»), то есть в список файлов, которые войдут в коммит. 

*Staging area также называют index (англ. «каталог») или cache (англ. «кеш»), а состояние файла `staged` иногда называют `indexed` или `cached`.*

*`tracked` - противоположность untracked. Оно довольно широкое по смыслу: в него попадают файлы, которые уже были зафиксированы с помощью `git commit`, а также файлы, которые были добавлены в staging area командой `git add`. То есть все файлы, в которых Git так или иначе отслеживает изменения.
...
*Может сложиться ситуация, что файл одновременно `staged` и `modified`. Такое возможно, если файл добавлен в Staging area командой `git add`, а затем был изменен, тогда гит учитывает две версии файла. Чтобы подтянуть изменения необходимо повторно выполнить `git add`*
